...
| 版本标识 | 类型 | 当前状态 | 说明 |
|---|---|---|---|
| `LEGACY-FE-8083` | 前端保留版本 | 保留使用 | 当前 `101.43.49.78:8083` 的 Vue + Vite 前端入口，作为页面适配和联调入口 |
| `API-CURRENT-0.1` | 当前已落地 API | 可运行 | 当前 FastAPI 已落地的最小闭环接口，包含设备状态、模拟器、Dashboard、Nacos 等基础能力 |
...
| `OPENAPI-8082-CURRENT-2026.05.07` | OpenAPI 快照 | 当前参考 | 当前文档站发布的 `openapi.json` 快照 |
| `DOC-2026.05.07` | 文档版本 | 当前文档集 | 当前文档站所有开发文档的统一版本日期 |
...
| 当前能不能联调 | 可以，先用 `API-CURRENT-0.1` 和 `SIM-0.1` 联调，真实硬件按 `SIM-0.2` 接入 |
